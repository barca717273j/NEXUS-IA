...
interface ProjectWizardProps {
  onGenerate: (name: string, niche: string, objective: string, pages: number, language: string, coverUrl?: string, type?: "ebook" | "landing_page" | "site") => void;
...
export default function ProjectWizard({ onGenerate, isGenerating, mode = "ebook" }: ProjectWizardProps) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    onGenerate(name, niche, objective, pages, language, coverOption === "custom" ? customCoverUrl : undefined, mode);
  };

  const getHeaderTitle = () => {
    if (mode === "landing_page") return "Desenvolver Nova Landing Page de Alta Conversão";
    if (mode === "site") return "Desenvolver Novo Site Institucional de Apoio";
    return "Estruturar Novo Ebook Premium & Recursos";
  };

  const getHeaderDescription = () => {
    if (mode === "landing_page") {
      return "Crie uma página de vendas magnética com headline irresistível, seções de problemas, soluções, depoimentos, FAQs e garantias editáveis. Preencha as premissas abaixo para iniciar.";
    }
    if (mode === "site") {
      return "Crie um portal institucional completo com seção sobre nós, canais de contato, diferenciais de marca e perguntas frequentes. Preencha as premissas abaixo para iniciar.";
    }
    return "Nossa inteligência avançada mapeia o mercado, desenha o avatar ideal, constrói a estrutura pedagógica completa de capítulos do seu livro e gera scripts de copy validados. Preencha as premissas abaixo para iniciar com segurança.";
  };

  const getNameLabel = () => {
    if (mode === "landing_page") return "Nome do Produto / Oferta";
    if (mode === "site") return "Nome da Marca / Empresa";
    return "Título do Ebook (Nome sugerido)";
  };

  const getNamePlaceholder = () => {
    if (mode === "landing_page") return "Exemplo: Método Liberdade Financeira 30D";
    if (mode === "site") return "Exemplo: Nexus Operações";
    return "Exemplo: Manual da Riqueza Conservadora";
  };
        ...
        {!isGenerating ? (
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <label className="text-[10px] font-mono uppercase text-zinc-400 tracking-wider font-bold flex items-center gap-2">
                    ...
                    Objetivo Comercial & Proposta de Valor
                  </label>
                ...
                <textarea
                  ...
                  placeholder="Seja descritivo. Exemplo: Quero criar uma oferta focada em ensinar profissionais a escalar suas vendas usando tráfego orgânico estruturado passo a passo, sem custos de anúncios."
                  rows={4}
                ...
                <p className="text-[10px] text-zinc-500 leading-normal">
                  Dica: Quanto mais detalhes sobre o público-alvo e a solução, mais personalizada e assertiva será a geração com inteligência artificial.
                </p>
              </div>
              ...
              {/* Advanced Parameters Section (Only for Ebook mode) */}
              {mode === "ebook" && (
              ...
              {/* Cover Design Settings (Only for Ebook mode) */}
              {mode === "ebook" && (
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    >
                      <p className="text-xs font-bold text-white mb-1 flex items-center gap-1.5">
                        ...
                        Capa Editorial Dinâmica
                      </p>
                      <p className="text-[10px] text-zinc-500 leading-normal">Visual editorial tipográfico profissional adaptado ao seu nicho com gradiente.</p>
                    </button>
              ...
              <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-6 border-t border-zinc-800/80">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-zinc-500 font-mono">Consome <span className="text-red-500 font-black">1 crédito corporativo</span> • Processamento Seguro</p>
                  </div>
                </div>
                ...
                >
                  ...
                  <span>
                    {mode === "landing_page" ? "Gerar Landing Page" : mode === "site" ? "Gerar Site Institucional" : "Gerar Ebook & Recursos"}
                  </span>
          ...
          >
            ...
            <h3 className="font-serif text-2xl sm:text-3xl font-extrabold text-white mb-4 tracking-tight">
              {mode === "landing_page" ? "Gerando Landing Page de Alta Conversão" : mode === "site" ? "Gerando Site Institucional" : "Gerando Ebook & Recursos Nexus"}
            </h3>
            
            {/* Elegant scrolling loader letters - responsive and wrap-safe */}
            <div className="min-h-[3.5rem] py-2 max-w-lg mb-8 flex items-center justify-center overflow-visible">
              <AnimatePresence mode="wait">
                <motion.p
                  ...
                  className="text-xs sm:text-sm text-red-500 font-mono tracking-wide font-black uppercase text-center drop-shadow-[0_0_10px_rgba(239,68,68,0.2)] px-4 leading-relaxed"
                >
